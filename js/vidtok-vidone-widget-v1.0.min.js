jQuery(document).ready(function(c){var b=jQuery.url().param("vid");if("undefined"==b||null==b)b="";jQuery("body").append('<div class="vidone-widget-w"> <div class="vidone-widget-invite-email"> <div id="vidone-invalid-email">Oops! Not a valid email</div> <p>Invite friend to join video chat</p> <form id="vidone-invite-form" name="vidone-invite-form" method="post"> <input type="text" name="email" id="email" value="" /> <input type="submit" name="vidone-invite-submit" id="vidone-invite-submit" value="Send Invite" class="invite btn btn-info" /> </form> </div> <div class="vidone-widget-video-w"> <div class="vidone-widget-video-h" id="pub"> <div id="vidone-pub"><img src="'+
VIDONE_PLUGIN_URL+'img/large-loader-cyan.gif" alt="loading" /><p>Loading Video Connection...</p></div> </div> <div class="vidone-widget-video-h" id="sub"> <div id="vidone-sub"><p>Your guest will appear here...</p></div> </div> <div class="vidone-widget-access"> <div class="vidone-widget-start"> <p>Want to start a new video chat?</p> <input type="button" class="start btn btn-info" id="create-button" value="Create Video Chat" /> </div> <div class="vidone-widget-join"> <div id="vidone-invalid">Oops! Wrong code, try again</div> <p>Join an existing video chat</p> <form id="vidone-join-form" name="vidone-join-form" method="post"> <input type="text" name="vid" id="vid" value="'+
b+'" /> <input type="submit" name="vidone-join-submit" id="vidone-join-submit" value="Join Existing Video Chat" class="join btn btn-info" /> </form> </div> </div> </div> <div class="vidone-widget-chat-bubble" id="bubble"><img src="'+VIDONE_PLUGIN_URL+'img/video-chat-bubble.png" alt="vidtok vid {one} chat bubble"/></div> <div class="vidone-widget-action-bar"> <div id="vidone-widget-default-view"> <div class="vidone-start-msg vidone-widget-default-msg-size"> <div class="vidone-widget-action-bar-arrow" id="up-arrow"> <img src="'+
...
VIDONE_PLUGIN_URL+'img/down-arrow.png" alt="vidtok vid {one} end"/></div> <p>End your video chat</p> </div> </div> </div> </div>');invite=clicked=start=0;jQuery(".vidone-start-msg").show();jQuery("#vid").watermark("Enter Invitation Code");jQuery("#email").watermark("Enter Email Address");setTimeout(function(){jQuery(".vidone-widget-w").animate({right:"20px"})},750);jQuery.url().param("vid")&&setTimeout(function(){jQuery("#vidone-invalid").hide();jQuery("#vidone-widget-default-view").empty();jQuery("#vidone-widget-default-view").append('<div class="vidone-close-msg vidone-widget-default-msg-size"> <div class="vidone-widget-action-bar-arrow" id="down-arrow"> <img src="'+
VIDONE_PLUGIN_URL+'img/down-arrow.png" alt="vidtok vid {one} close" /> </div> <p>Close video start window</p> </div>');jQuery(".vidone-close-msg").fadeIn("slow");jQuery(".vidone-widget-chat-bubble").animate({bottom:"-200px"},375,function(){jQuery(".vidone-widget-access").animate({bottom:"390px"});clicked=0});jQuery.ajax({url:vidoneAjax.ajaxurl,type:"POST",data:{action:"join_session",vid:b},dataType:"json",success:function(a){"non-existing"===a.status&&jQuery("#vidone-invalid").fadeIn();"exists"===
a.status&&(jQuery.ajax({url:"http://vidtok.co/vidone/usage",type:"POST",data:{vapi:VAPI,domain:DOMAIN,type:"join"}}),jQuery(".vidone-widget-invite").hide(),jQuery(".vidone-widget-get-vidone").show(),jQuery("#vidone-invalid").hide(),vidone(),VID=a.vid,TOKBOX_SESSION_ID=a.session_id,TOKBOX_TOKEN=a.token,setTimeout(function(){TB.setLogLevel(TB.DEBUG);session=TB.initSession(TOKBOX_SESSION_ID);session.addEventListener("sessionConnected",sessionConnectedHandler);session.addEventListener("streamCreated",
streamCreatedHandler);session.addEventListener("streamDestroyed",streamDestroyedHandler);session.connect("21436142",TOKBOX_TOKEN);jQuery("#controller_"+TOKBOX_SESSION_ID).css({width:"0px",height:"0px"})},2500))}})},1750);jQuery("div").on("click","#up-arrow",function(a){a.preventDefault();0==clicked&&(access(),clicked++)});jQuery("div").on("click","#bubble",function(a){a.preventDefault();0==clicked&&(access(),clicked++)});jQuery("div").on("click","#down-arrow",function(a){a.preventDefault();0==clicked&&
(cAccess(),clicked++)});jQuery("div").on("click","#end-arrow",function(a){a.preventDefault();0==clicked&&(cVidone(),clicked++)});jQuery("a[rel='invite-email']").toggle(function(a){a.preventDefault();jQuery(".vidone-widget-invite-email").animate({bottom:"80px"})},function(a){a.preventDefault();jQuery("#vidone-invalid-email").hide();jQuery(".vidone-widget-invite-email").animate({bottom:"-140px"})});jQuery("#vidone-invite-form").on("submit",function(a){a.preventDefault();a=jQuery("#email").val();jQuery.ajax({url:vidoneAjax.ajaxurl,
type:"POST",data:{action:"invite_email",vid:VID,email:a,url:VIDONE_INVITE_URL},dataType:"json",success:function(a){"invalid"===a.status&&jQuery("#vidone-invalid-email").fadeIn();"valid"===a.status&&(jQuery("#vidone-invalid-email").hide(),jQuery(".vidone-widget-invite-email").animate({bottom:"-140px"}))}})});jQuery("#vidone-join-form").on("submit",function(a){a.preventDefault();jQuery("#vidone-invalid").hide();a=jQuery("#vid").val();jQuery.ajax({url:vidoneAjax.ajaxurl,type:"POST",data:{action:"join_session",
vid:a},dataType:"json",success:function(a){"non-existing"===a.status&&jQuery("#vidone-invalid").fadeIn();"exists"===a.status&&(jQuery.ajax({url:"http://vidtok.co/vidone/usage",type:"POST",data:{vapi:VAPI,domain:DOMAIN,type:"join"}}),jQuery(".vidone-widget-invite").hide(),jQuery(".vidone-widget-get-vidone").show(),jQuery("#vidone-invalid").hide(),vidone(),VID=a.vid,TOKBOX_SESSION_ID=a.session_id,TOKBOX_TOKEN=a.token,setTimeout(function(){TB.setLogLevel(TB.DEBUG);session=TB.initSession(TOKBOX_SESSION_ID);
session.addEventListener("sessionConnected",sessionConnectedHandler);session.addEventListener("streamCreated",streamCreatedHandler);session.addEventListener("streamDestroyed",streamDestroyedHandler);session.connect("21436142",TOKBOX_TOKEN);jQuery("#controller_"+TOKBOX_SESSION_ID).css({width:"0px",height:"0px"})},2500))}})});jQuery("div").on("click",".start",function(a){0==start&&(jQuery(".vidone-widget-get-vidone").hide(),jQuery(".vidone-widget-invite").show(),vidone(),jQuery.ajax({url:vidoneAjax.ajaxurl,
type:"POST",data:{action:"create_session"},dataType:"json",success:function(a){jQuery.ajax({url:"http://vidtok.co/vidone/usage",type:"POST",data:{vapi:VAPI,domain:DOMAIN,type:"create"}});VID=a.vid;TOKBOX_SESSION_ID=a.session_id;TOKBOX_TOKEN=a.token;setTimeout(function(){TB.setLogLevel(TB.DEBUG);session=TB.initSession(TOKBOX_SESSION_ID);session.addEventListener("sessionConnected",sessionConnectedHandler);session.addEventListener("streamCreated",streamCreatedHandler);session.addEventListener("streamDestroyed",
streamDestroyedHandler);session.connect("21436142",TOKBOX_TOKEN);jQuery("#controller_"+TOKBOX_SESSION_ID).css({width:"0px",height:"0px"})},750)}}),start++)})});
